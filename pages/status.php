...
<div class="container mx-auto px-4 sm:px-6 lg:px-8 max-w-3xl">
    <div class="text-center mb-8">
        <div class="w-16 h-16 rounded-2xl gradient-brand flex items-center justify-center text-white text-2xl mx-auto mb-4 shadow-lg"><i class="fa-solid fa-magnifying-glass"></i></div>
        <h1 class="text-3xl sm:text-4xl font-extrabold text-slate-900">Cek <span class="gradient-text">Status Lamaran</span></h1>
        <p class="text-slate-600 mt-3">Masukkan kode tracking yang Anda terima setelah submit lamaran.</p>
    </div>
    <form id="form-track" class="bg-white rounded-2xl shadow-lg border border-slate-100 p-3 flex gap-2">
        <div class="relative flex-1">
            <i class="fa-solid fa-hashtag absolute left-4 top-1/2 -translate-y-1/2 text-slate-400"></i>
            <input id="kode" type="text" placeholder="Contoh: BPR-20260514-ABC123" required class="w-full pl-11 pr-4 py-3 rounded-xl bg-slate-50 border border-slate-200 focus:border-brand-500 focus:ring-2 focus:ring-brand-100 outline-none text-sm font-semibold uppercase">
        </div>
        <button class="px-6 py-3 rounded-xl gradient-brand text-white font-bold whitespace-nowrap"><i class="fa-solid fa-search mr-1"></i> Cek</button>
    </form>
    ...
    <div class="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden animate-fade-in-up">
        ...
        <div class="p-6">
            <dl class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm mb-6">
                <div><dt class="text-slate-500">Nama</dt><dd class="font-bold text-slate-900 mt-0.5">${d.nama_lengkap}</dd></div>
                <div><dt class="text-slate-500">Email</dt><dd class="font-bold text-slate-900 mt-0.5 truncate">${d.email}</dd></div>
                <div><dt class="text-slate-500">Posisi</dt><dd class="font-bold text-slate-900 mt-0.5">${d.posisi_dilamar}</dd></div>
                <div><dt class="text-slate-500">Penempatan</dt><dd class="font-bold text-slate-900 mt-0.5">${d.penempatan}</dd></div>
                <div><dt class="text-slate-500">Tanggal Melamar</dt><dd class="font-bold text-slate-900 mt-0.5">${APP.formatDate(d.created_at)}</dd></div>
                <div><dt class="text-slate-500">Update Terakhir</dt><dd class="font-bold text-slate-900 mt-0.5">${APP.formatDate(d.updated_at)}</dd></div>
            </dl>
            ...
            :`<div class="relative">
                <div class="hidden sm:block absolute top-6 left-[8%] right-[8%] h-1 bg-slate-100 rounded-full z-0"><div class="h-full gradient-brand rounded-full transition-all" style="width:${(activeIdx/(stages.length-1))*100}%"></div></div>
                <div class="relative z-10 grid grid-cols-3 sm:grid-cols-6 gap-2">
                    ${stages.map((s,i)=>{
                        const done=i<=activeIdx;const active=i===activeIdx;
                        return`<div class="text-center"><div class="w-10 h-10 sm:w-12 sm:h-12 mx-auto rounded-full flex items-center justify-center text-sm mb-1 ${done?'gradient-brand text-white shadow-lg':'bg-slate-100 text-slate-400'} ${active?'ring-4 ring-brand-200':''}"><i class="fa-solid ${done?'fa-check':'fa-circle text-[6px]'}"></i></div><p class="text-[10px] font-bold ${done?'text-slate-900':'text-slate-400'}">${s.label}</p></div>`;
                    }).join('')}
                </div>
            </div>`}
            ${d.catatan_admin?`<div class="mt-6 bg-indigo-50 border border-indigo-100 rounded-xl p-4"><p class="text-xs font-bold text-brand-700 uppercase mb-1"><i class="fa-solid fa-note-sticky mr-1"></i>Catatan HR</p><p class="text-sm text-slate-700 whitespace-pre-line">${d.catatan_admin}</p></div>`:''}
        </div>
    </div>`;
});
})();
