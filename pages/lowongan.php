...
<div class="container mx-auto px-4 sm:px-6 lg:px-8 py-12 lg:py-16 relative z-10">
    ...
    <p class="text-slate-600 mt-3 max-w-2xl">Temukan posisi yang sesuai dengan keahlian Anda di PT BPR BKK Jateng (Perseroda).</p>
</div>
</section>
...
(async function(){
    ...
    if(res.status!==200||!res.data||res.data.length===0){
        listEl.innerHTML=`<div class="col-span-full text-center py-16 bg-white rounded-2xl border border-slate-100">
            ...
            <p class="text-sm text-slate-500 mt-1">Silakan cek kembali nanti.</p></div>`;
        return;
    }
    listEl.innerHTML=res.data.map(j=>{
        ...
        return`
        <a href="${APP.baseUrl}/detail/${j.id}" class="group bg-white rounded-2xl p-6 border border-slate-100 hover:border-brand-300 hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
            <div class="flex flex-col md:flex-row md:items-center gap-4">
                <div class="w-16 h-16 rounded-2xl gradient-brand flex items-center justify-center text-white text-2xl shadow-lg shrink-0">
                    <i class="fa-solid fa-building-columns"></i>
                </div>
                <div class="flex-1 min-w-0">
                    <h3 class="text-xl font-extrabold text-slate-900 group-hover:text-brand-700 transition">${j.judul}</h3>
                    <p class="text-sm text-slate-600 mt-1 line-clamp-2">${j.deskripsi||''}</p>
                    <div class="flex flex-wrap gap-2 mt-3">
                        ...
                        <span class="text-[11px] font-semibold px-2.5 py-1 rounded-full bg-amber-50 text-amber-700"><i class="fa-solid fa-calendar mr-1"></i>Deadline: ${deadline}</span>
                        <span class="text-[11px] font-semibold px-2.5 py-1 rounded-full bg-emerald-50 text-emerald-700"><i class="fa-solid fa-briefcase mr-1"></i>${(j.posisi_tersedia||[]).length} Posisi</span>
                    </div>
                    <p class="text-xs text-slate-500 mt-2"><i class="fa-solid fa-list-check mr-1"></i>${posisi}${more}</p>
                </div>
                <div class="shrink-0">
                    <span class="inline-flex items-center gap-1 px-4 py-2 rounded-xl bg-brand-50 text-brand-700 font-bold text-sm group-hover:gradient-brand group-hover:text-white transition">
