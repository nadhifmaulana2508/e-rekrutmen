...
<div class="bg-white rounded-2xl border border-slate-100 p-4 flex flex-col sm:flex-row gap-3 items-stretch sm:items-center justify-between">
    <div class="relative flex-1 max-w-md">
        ...
        <input id="search" type="text" placeholder="Cari judul / divisi / lokasi..." class="input pl-10">
    </div>
...
<div class="bg-white rounded-2xl border border-slate-100 mt-5 overflow-hidden">
    <div class="overflow-x-auto">
        <table class="min-w-full text-sm">
            ...
            <tbody id="rows">
                <tr><td colspan="8" class="py-12 text-center text-slate-400">Memuat data...</td></tr>
            </tbody>
        </table>
    </div>
</div>
...
(async function () {
    ...
    async function load() {
        rows.innerHTML = `<tr><td colspan="8" class="py-12 text-center text-slate-400">Memuat data...</td></tr>`;
        const res = await ADMIN.api('/lowongan?all=1');
        if (res.status !== 200) {
            rows.innerHTML = `<tr><td colspan="8" class="py-12 text-center text-rose-500">${res.message}</td></tr>`;
            return;
        }
        all = res.data || [];
        render();
    ...
    function render() {
        ...
        const list = all.filter(l => {
            ...
            if (q) {
                const t = (l.judul + ' ' + l.divisi + ' ' + l.lokasi).toLowerCase();
        ...
        if (list.length === 0) {
            rows.innerHTML = `<tr><td colspan="8" class="py-12 text-center text-slate-400">
        ...
        rows.innerHTML = list.map(l => `
            <tr class="border-b border-slate-50 hover:bg-slate-50/60 transition">
                <td class="px-4 py-3">
                    ...
                    <p class="text-xs text-slate-500 mt-0.5">${ADMIN.formatLevel(l.level)}</p>
                </td>
                <td class="px-4 py-3 text-slate-700">${l.divisi}</td>
                <td class="px-4 py-3 text-slate-600">${l.lokasi}</td>
                <td class="px-4 py-3 text-slate-600">${ADMIN.formatTipe(l.tipe_kerja)}</td>
        ...
        rows.querySelectorAll('.btn-del').forEach(btn => {
            btn.addEventListener('click', async () => {
                ...
                const ok = await ADMIN.confirm(`Hapus lowongan <b>${judul}</b>? Semua pelamar yang terkait juga akan dihapus.`, { danger: true, okText: 'Hapus' });
                if (!ok) return;
