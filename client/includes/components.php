...
function renderSidebar(string $page, array $user): void {
    ...
    <aside id="sidebar" class="fixed lg:static inset-y-0 left-0 z-40 w-72 bg-white border-r border-slate-200 flex flex-col transform -translate-x-full lg:translate-x-0 transition-transform duration-300">
        ...
        <nav class="flex-1 overflow-y-auto p-4 space-y-1">
            <p class="text-[10px] uppercase tracking-wider text-slate-400 font-bold px-3 mb-2">Menu Utama</p>
            <?php foreach ($menu as [$key, $label, $icon]):
                $active = $page === $key || ($key === 'pelamar' && $page === 'pelamar_detail') || ($key === 'lowongan' && $page === 'form_lowongan');
                $active = $page === $key;
            ?>
                <a href="<?= $base ?>/<?= $key ?>"
                   class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-semibold transition-all
                    ...
                    <i class="fa-solid <?= $icon ?> w-5 text-center <?= $active ? '' : 'text-slate-400' ?>"></i>
                    <span><?= $label ?></span>
                </a>
            <?php endforeach; ?>

            <div class="mt-6 p-4 rounded-2xl bg-gradient-to-br from-indigo-50 to-pink-50 border border-indigo-100">
                <p class="text-xs font-bold text-brand-800"><i class="fa-solid fa-lightbulb mr-1"></i> Info</p>
                <p class="text-xs text-slate-600 mt-1 leading-relaxed">Update status pelamar secara berkala agar kandidat terinformasi dengan baik.</p>
            </div>
        </nav>
        ...
        <div class="p-4 border-t border-slate-100 shrink-0">
            <div class="flex items-center gap-3 p-2 rounded-xl bg-slate-50">
                <div class="w-10 h-10 rounded-full gradient-brand flex items-center justify-center text-white font-bold"><?= $initial ?></div>
                ...
                <a href="<?= $base ?>/dashboard?logout=1" class="w-9 h-9 rounded-lg bg-white hover:bg-rose-50 hover:text-rose-600 text-slate-500 flex items-center justify-center transition" title="Logout">
